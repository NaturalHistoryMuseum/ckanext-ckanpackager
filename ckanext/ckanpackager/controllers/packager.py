...
class CkanPackagerController(t.BaseController):
    ...
    def _validate_request(self):
        ...
        if u'anon' in t.request.params:
            raise PackagerControllerError(_(u'You must be logged on or have javascript enabled to use this functionality.'))
        if t.c.user and u'email' in t.request.params:
            raise PackagerControllerError(_(u'Parameter mismatch. Please reload the page and try again.'))
    ...
    def package_resource(self, package_id, resource_id):
        ...
        try:
            ...
            if u'message' in result:
                ...
                flash_success(_(u'Request successfully posted. The resource should be emailed to you shortly'))
    ...
    def _prepare_packager_parameters(self, email, resource_id, params):
        ...
        if resource.get(u'datastore_active', False):
            ...
            for option in [u'filters', u'q', u'limit', u'offset', u'resource_url', u'sort']:
                if option in params:
                    if option == u'filters':
                        request_params[u'filters'] = json.dumps(self._parse_filters(params[u'filters']))
                    else:
    ...
    def _send_packager_request(self, packager_url, request_params):
        ...
        try:
            ...
            raise PackagerControllerError(_(u'Failed to contact the ckanpackager service'))
        ...
        try:
            ...
            raise PackagerControllerError(_(u'Could not parse response from ckanpackager service'))
        finally:
            response.close()
        return result
